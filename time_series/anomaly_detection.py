...
def ARMA_outliar(signal):
    model = ARIMA(signal, order = (4,0,10))
    res = model.fit()
...
def polyreg_outliar_mse(arr, n):
    ...
    for t, _ in enumerate(arr):
        ...
        #     for i in range(int(n/2)):
        #         ret.append(reg.predict(np.array([i]).reshape(1, -1)) - arr[i])
        ret.append(np.mean((reg.predict(loc) - arr[t-int(n/2):t+int(n/2)])**2))
    return ret
...
if __name__ == "__main__":
    ...
    # plot_acf(np.array(derivative(signal)))
    # plt.show()
    fig, axs = plt.subplots(6)
    axs[0].plot(signal)
    axs[1].plot(lof(signal, 100))
    axs[2].plot(np.abs(polyreg_outliar_mse(signal, 30)))
    axs[3].plot(ARMA_outliar(signal))
